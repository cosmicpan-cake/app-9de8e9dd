(function () {
  ...
  // Its own key: this screen holds different stocks from the Garanti one it was
  // copied from, and sharing a key would let each overwrite the other.
  var STORAGE_KEY = "portfoyDemo2Stocks_v1";
  ...
  // Both sit 370px above the Garanti screen's, the height of the "Hisse Senedi
  // Varlığım" card this screen leaves out.
  var CARD_GROUP_TOP = 669;
  var FIRST_ICON_TOP = 694;
  ...
  /* The average costs carry more precision than the screen prints. The source
     shows KUYAS at 68,00 with a loss of 2.656.765,83 - which 68,00 exactly
     cannot produce - so the figures are struck on the unrounded cost the app
     really holds, and the rows round it for display. Same for RALYH. */
  var DEFAULT_STOCKS = [
    {
      id: "KUYAS",
      ...
      fiyat: 56.450,
      degisim: -9.97,
      ortMaliyet: 68.0011557826,
      visible: true
    },
    {
      id: "RALYH",
      name: "RALYH - RAL YATIRIM HOLDİNG ANONİM ŞİRKET",
      nameLines: ["RALYH - RAL YATIRIM HOLDİNG", "ANONİM ŞİRKET"],
      iconKind: "ralyh",
      adet: 30000,
      fiyat: 220.000,
      degisim: -7.76,
      ortMaliyet: 258.109258,
      visible: true
    }
  ];

  /* No global price hook here: the launcher's shared figure is an IEYHO price,
     and neither holding on this screen is IEYHO. */
  ...
  /* Without the "Hisse Senedi Varlığım" card there is no portfolio total to
     write, so the account section is pure image and nothing is drawn over it. */

  /* Only the holdings that fit above the bottom bar are drawn — the bar itself
     never moves. */
  var NAV_TOP = 1850;
